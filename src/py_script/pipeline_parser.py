...
def add_permutations_arg(parser):
    parser.add_argument(
        "--permutations",
        dest="n_permutations",
        required=True,
        type=int,
        help="Number of phenotype permutations (must be >= 1)"
    )
# Files:
# phenotype file
...
# gwas software
# Add this line after defining other arguments
# Paths to executables
parser.add_argument("--gemma_path", default="gemma",
        help="Path to GEMMA executable [default: gemma in PATH]")
parser.add_argument("--rscript_exec", default="Rscript", help="Path to Rscript executable [default: Rscript]")
# Covariates
parser.add_argument("--covariates", default=None, help="Covariate file (optional; tab-delimited, same sample order)")
parser.add_argument("--dry_run", action="store_true", help="Print commands but do not execute external tools")

add_permutations_arg(parser)
args = parser.parse_args()
if getattr(args, "n_permutations", 0) < 1:
    raise SystemExit("ERROR: --permutations must be >= 1 when permutation is required.")
if getattr(args, "n_permutations", 0) < 1:
    raise SystemExit("ERROR: --permutations must be >= 1 when permutation is required.")
## === Deprecation warnings for post-processing flags (kept for backward compatibility) ===
#for _name, _msg in _deprecated_flags:
#    if hasattr(args, _name):
#        _val = getattr(args, _name)
#        if isinstance(_val, bool) and _val:
#            print("[DEPRECATED]", _name, "->", _msg)
